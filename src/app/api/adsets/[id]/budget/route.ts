/**
 * PATCH /api/adsets/[id]/budget
 * Update ad set budget (daily or lifetime)
...
export async function PATCH(
  ...
  try {
    ...
    // Collect tokens (same pattern as adset toggle)
    ...
    if (user?.metaAccount?.accessToken) {
      try {
        ...
        console.error('[adset-budget] Failed to decrypt token:', e);
      }
    }
    if (user?.accounts) {
    ...
    if (memberRecord?.userId) {
      ...
      if (teamOwner?.metaAccount?.accessToken) {
        try {
          ...
          console.error('[adset-budget] Owner token decrypt failed:', e);
        }
      }
    ...
    if (!res.ok) {
      ...
      return NextResponse.json(
        { error: err.error?.message || 'Failed to update ad set budget' },
        { status: res.status }
      );
    }
    ...
    console.error('[adset-budget] Error:', error);
    return NextResponse.json(
