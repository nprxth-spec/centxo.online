  ...
  try {
    ...
    if (user?.metaAccount?.accessToken) {
      try {
        ...
        console.error('[adset-toggle] Failed to decrypt MetaAccount token:', e);
      }
    }
    ...
    if (memberRecord?.userId) {
      ...
      console.log('[adset-toggle] User is team member, fetching owner tokens from:', teamOwnerId);
      ...
      if (teamOwner?.metaAccount?.accessToken) {
        try {
          ...
          console.error('[adset-toggle] Failed to decrypt team owner token:', e);
        }
      }
    ...
    console.log('[adset-toggle] Found tokens:', tokens.length);
